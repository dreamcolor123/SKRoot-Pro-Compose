...
> 本项目保留原 SKRoot Pro 的 Native/JNI、AIDL、Magica 服务、JSON 协议、应用 ID 和权限。4.5.9 使用上游 APK 中经过哈希校验的 ARM64 Native 文件；相关实现与资源归属原项目作者。
...
应用版本使用 `<上游核心版本>.<UI 修订号>`，当前为 `4.5.9.1`。上游核心更新时同步前三段并将 UI 修订号重置为 `1`；本项目每次发布时递增最后一段。
...
```text
app/build/outputs/apk/release/v4.5.9.1-UI重构版-SKRoot Pro.apk
```
...
最新公开 Release [v4.5.9.1](https://github.com/dreamcolor123/SKRoot-Pro-Compose/releases/tag/v4.5.9.1) 提供签名正式版 APK：

- [`v4.5.9.1-UI重构版-SKRoot Pro.apk` / `com.linux.compose`](https://github.com/dreamcolor123/SKRoot-Pro-Compose/releases/download/v4.5.9.1/v4.5.9.1-UI%E9%87%8D%E6%9E%84%E7%89%88-SKRoot%20Pro.apk)

SHA-256：

```text
ea0a73662daabeb1fb50dd5179aadb1835cb26287ae7cd3fc0a023b39ce4b4e7  v4.5.9.1-UI重构版-SKRoot Pro.apk
```
...
本版本主要包含 UI、状态管理、导航、主题和构建迁移。Native、JNI、AIDL、Magica 服务和现有业务协议跟随上游语义；4.5.9 的四项 ARM64 Native 文件直接同步自上游 APK并进行 SHA-256 构建校验。
